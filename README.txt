...
The script will take the template badge in badge_template.png and the list of name;company;language tuple (separated by ;) in people.csv and will generate badges for each person. The script will create a directory called "people" and put the final badge images there.
...
* Font names, sizes, colours and locations are hardcoded at the moment. A future version will parametrise them. For now, you'll need to delve into the code to change these values. Here are some pointers:
    * Roboto-Medium.ttf is Roboto from Google Font. Change this if you want some other font. If you are using a non-truetype font, you'll need to change the ImageFont.truetype call. PIL supports various font formats. See http://www.pythonware.com/library/pil/handbook/imagefont.htm for help on how to use other font formats.
